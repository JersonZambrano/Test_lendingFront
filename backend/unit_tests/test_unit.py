...
import ports.requestedLoanHandler as PORTHANDLER
...
class ServerTest(AsyncHTTPTestCase):
    def get_app(self):
        return tornado.web.Application([(r"/loan/(?P<action>[A-Za-z]+)?", PORTHANDLER.RequestedLoanHandler)], debug=False)
...
class LoanTest(ServerTest):
    ...
    def test_without_requested_amount(self):
        ...
        try:
            result = self.post("/loan/register", loan_register)
            self.assertIn("loan_decision", result)
            self.assertEqual(result["loan_decision"],
                             "Requested amount parameter does not exists")
        except ValueError as error:
            console("Error trace: %s" % traceback.format_exc())
